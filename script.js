...
(function () {
  ...
  function hideAll() {
    ...
    lbLink.style.display = 'none'; lbLink.href = '';
  }

  function showImg(src, caption, desc, id, link) {
    hideAll();
    lbImg.src = src;
    lbImg.style.display = '';
  ...
  document.querySelectorAll('.dsgn-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      showImg(img.src, title, card.dataset.desc || '', card.dataset.id, card.dataset.link);
    });
  });
  ...
  // Deep link: open project or scroll to section from URL hash on page load
  ...
  if (hash) {
    ...
    if (target) {
      ...
      if (section) setTimeout(() => section.scrollIntoView({ behavior: 'smooth' }), 700);
    }
  }
})();
